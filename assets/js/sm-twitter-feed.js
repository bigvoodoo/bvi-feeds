...
jQuery(function ($) {
    $(".twitter-feed").each(function () {
        ...
        var post_template = $.trim(
            template.replace(
                /[\s\S]*\{tweets\}([\s\S]+)\{\/tweets\}[\s\S]*/,
                "$1"
            )
        );
        template = $.trim(
            template.replace(/\{tweets\}[\s\S]+\{\/tweets\}/, "{tweets}")
        );
        container.empty();
        ...
        var uri =
            ...
            id +
            "?do_not_track=true&lang=en&hide_at_replies=true&suppress_response_codes=true&rnd=" +
            Math.random() +
            "&callback=?";
        ...
        $.getJSON(uri, null, function (timeline_html) {
            ...
            if (!timeline.find("li.timeline-TweetList-tweet").length) {
                ...
                fakeTweet.append(
                    '<div class="e-entry-content"><div class="e-entry-title">No tweets found...</div></div>'
                );
                fakeTweet.append(
                    $("<time>").attr("datetime", new Date().toISOString())
                );
                fakeTweet.append(
                    '<div><div class="reply-action web-intent"></div><div class="retweet-action web-intent"></div><div class="favorite-action web-intent"></div></div>'
                );
            ...
            timeline.find("li.timeline-TweetList-tweet").each(function () {
                ...
                var tweet = {
                    ...
                    url: $(this)
                        .find("a.timeline-Tweet-timestamp")
                        .attr("href"),
                    content: sm_twitter_clean_content(
                        $(this).find(".timeline-Tweet-text"),
                        shorten_links
                    ).html(),
                    time: sm_twitter_format_date(
                        moment($(this).find("time").attr("datetime")),
                        use_timeago
                    ),
                };
